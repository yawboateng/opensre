...
Supported env vars: `GRAFANA_INSTANCES`, `DD_INSTANCES`, `GROUNDCOVER_INSTANCES`, `HONEYCOMB_INSTANCES`, `CORALOGIX_INSTANCES`, `AWS_INSTANCES`, `ARGOCD_INSTANCES`, `SPLUNK_INSTANCES`, `KUBERNETES_INSTANCES`.
...
- Alert-hint selection (`grafana_instance`) is Grafana-only; Kubernetes instead selects per tool call via the `cluster` argument. Extending alert-hint selection to other providers is a follow-up.
