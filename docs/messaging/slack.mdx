...
| `OPENSRE_AGENT_NAME` | Name the bot introduces itself with, signs its footer with, and answers to in a thread. Defaults to `OpenSRE`. |

<Note>
Renaming the Slack app does not rename the agent — set `OPENSRE_AGENT_NAME` to
match. OpenSRE cannot read the name from Slack: a bot user's profile keeps the
old one until the app is reinstalled. See
[Agent identity](/configuration/environment-variables#agent-identity).
</Note>
